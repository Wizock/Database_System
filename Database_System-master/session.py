...
positions = ['Manager', 'chef', 'kitchen_hand', 'food packers', "checkout_operators", "delivery_drivers", "cleaners"]
connection = connect()
...
def edit():
    ...
    if registerNother == "yes":
        ...
        connection.execute('''
        INSERT INTO employee_personal (firstName,lastName,email,userName,Age,position)VALUES (?,?,?,?,?,?)''',
        ...
        userID = connection.execute('''
            SELECT UserID FROM employee_personal where userName = ?
            ''',(register_userName,))
        userID = list(userID)
        
        connection.execute('''
            INSERT INTO application_infomation (UserID,Username,password,email)
            VALUES (?,?,?,?)
            ''',
            (str(userID[0]),userName(),password(), register_email))
        connection.commit()
        connection.execute('''
            INSERT INTO employee_pay (UserID,Username,weeklyPay,wage,age)
            VALUES (?,?,?,?,?)
            ''',
            (str(userID[0]),userName(),weeklyPay(), calculateWage(),register_age))
        connection.commit()


        connection.execute('''
            INSERT INTO employee_shifts (UserID,Positions,startingTime,endingTime,shiftsPerWeek)
            VALUES (?,?,?,?,?)
            ''',
            (str(userID[0]),register_position,register_start,register_end,register_spw))
        connection.commit()


        connection.execute('''
            INSERT INTO late_arrivals (UserID,employeeName,startingTime,arrivalTime,lateBy,lateAmt)
            VALUES (?,?,?,?,?,?)
            ''',
            (str(userID[0]),register_firstName,register_start,arrivalTime(),late_factor,random.randint(0,15)))
        connection.commit()


        number = faker.phone_number()
        connection.execute('''
            INSERT INTO emergency_contact (UserID,Email,phoneNo)
            VALUES (?,?)
            ''',
            (str(userID[0]),register_email,number))
        connection.commit()
            
        connection.execute('''
            INSERT INTO normal_contact (UserID,Email,adress)
            VALUES (?,?)
            ''',
            (str(userID[0]),register_email,faker.address()))
